...
command_result df_showmood2 (color_ostream &out, vector <string> & parameters)
{
    if (!parameters.empty())
    ...
    for (df::job_list_link *cur = world->job_list.next; cur != NULL; cur = cur->next)
    {
        ...
        // iterate over all items necessary for this mood...
...
DFHACK_PLUGIN("showmood2");

DFhackCExport command_result plugin_init (color_ostream &out, std::vector<PluginCommand> &commands)
{
    commands.push_back(PluginCommand("showmood2", "Shows items needed for current strange mood.", df_showmood2, false,
